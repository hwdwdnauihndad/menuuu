...
function Menu.LoadBannerTexture(url)
    ...
    if CreateThread then
        CreateThread(function()
            ...
            if not success then
                print("Error loading banner texture: " .. tostring(result))
            end
        end)
    else
        local success, result = pcall(function()
    ...
    elseif item.type == "selector" and item.options then
        ...
        if isWardrobeSelector then
            ...
            if Susano and Susano.GetTextWidth then
                ...
                selectorWidth = string.len(selectorText) * 9
            end
            local selectorX = x + width - selectorWidth - 15
            Menu.DrawText(selectorX, textY, selectorText, selectorSize,
                1.0, 1.0, 1.0, 1.0)
        else
            local fullText = "< " .. selectedOption .. " >"
            local selectorWidth = 0
            ...
            Menu.DrawText(selectorX + leftArrowWidth, textY, selectedOption, selectorSize,
                1.0, 1.0, 1.0, 1.0)
...
function Menu.DrawCategories()
    if Menu.OpenedCategory then
        ...
        if currentTab and currentTab.items then
            ...
            local visibleHeight = math.min(maxVisible, totalItems) * itemHeight
        ...
        for i, tab in ipairs(Menu.TopLevelTabs) do
            ...
            if isSelected then
                ...
                for step = 0, gradientSteps - 1 do
                    ...
                    local alpha = (step / gradientSteps) * 0.5
                    
                    if Susano and Susano.DrawRectFilled then
                        Susano.DrawRectFilled(drawX, stepY, drawWidth, actualStepHeight, baseR, baseG, baseB, alpha, 0)
                    else
                         Menu.DrawRect(drawX, stepY, drawWidth, actualStepHeight, math.floor(baseR*255), math.floor(baseG*255), math.floor(baseB*255), math.floor(alpha*255))
                    end
                end
    ...
    for displayIndex = 1, math.min(maxVisible, totalCategories) do
        ...
        if categoryIndex <= #Menu.Categories then
            ...
            local textX = x + 15
            local textY = itemY + itemHeight / 2 - 8
            Menu.DrawText(textX, textY, category.name, 16, Menu.Colors.TextWhite.r / 255.0, Menu.Colors.TextWhite.g / 255.0, Menu.Colors.TextWhite.b / 255.0, 1.0)

            local chevronX = x + width - 20
            Menu.DrawText(chevronX, textY, ">", 16, Menu.Colors.TextWhite.r / 255.0, Menu.Colors.TextWhite.g / 255.0, Menu.Colors.TextWhite.b / 255.0, 1.0)
        end
    end
    ...
    if Menu.OpenedCategory then
        ...
        if category and category.hasTabs and category.tabs then
            ...
            if currentTab and currentTab.items then
                ...
                totalHeight = scaledPos.headerHeight + scaledPos.mainMenuHeight + scaledPos.mainMenuSpacing + (visibleItems * scaledPos.itemHeight)
            else
...
function Menu.DrawKeySelector(alpha)
    ...
    -- Fullscreen Overlay (Lighter Dim)
    if Susano and Susano.DrawRectFilled then
        Susano.DrawRectFilled(0, 0, screenWidth, screenHeight, 0.0, 0.0, 0.0, 0.6 * alpha, 0.0)
    else
        Menu.DrawRect(0, 0, screenWidth, screenHeight, 0, 0, 0, 180 * alpha)
    end

    -- Bar Dimensions
    local barWidth = 350
    local barHeight = 50
    local barX = math.floor((screenWidth / 2) - (barWidth / 2))
    local barY = math.floor(screenHeight - barHeight - 150)
    local barRadius = 6
    ...
    -- 1. Outline (Theme Color, Thin)
    local outlineThickness = 1
    if Susano and Susano.DrawRectFilled then
         Susano.DrawRectFilled(barX - outlineThickness, barY - outlineThickness, barWidth + (outlineThickness*2), barHeight + (outlineThickness*2), r, g, b, 1.0 * alpha, barRadius + outlineThickness)
    else
         Menu.DrawRect(barX - outlineThickness, barY - outlineThickness, barWidth + (outlineThickness*2), barHeight + (outlineThickness*2), math.floor(r * 255), math.floor(g * 255), math.floor(b * 255), 255 * alpha)
    end

    -- 2. Background (Dark, NO rose tint)
    if Susano and Susano.DrawRectFilled then
        Susano.DrawRectFilled(barX, barY, barWidth, barHeight, 0.0, 0.0, 0.0, 1.0 * alpha, barRadius)
    else
        Menu.DrawRect(barX, barY, barWidth, barHeight, 0, 0, 0, 255 * alpha)
    end

    -- Content
    local textY = barY + (barHeight / 2)
    local startX = barX + 30

    -- Spinner (Rotating Circle)
    local spinnerRadius = 8
    local spinnerThickness = 2.5
    local spinnerX = startX + spinnerRadius
    local spinnerY = textY
    
    local time = GetGameTimer() or 0
    local rotationSpeed = 0.4
    local rotationOffset = (time * rotationSpeed) % 360
    
    local segments = 12
    local arcAngle = 270 -- 3/4 circle
    
    for i = 0, segments do
        local currentAngle = rotationOffset + (i * (arcAngle / segments))
        local rad = math.rad(currentAngle)
        
        local px = spinnerX + spinnerRadius * math.cos(rad)
        local py = spinnerY + spinnerRadius * math.sin(rad)
        
        if Susano and Susano.DrawRectFilled then
             Susano.DrawRectFilled(px - spinnerThickness/2, py - spinnerThickness/2, spinnerThickness, spinnerThickness, r, g, b, 1.0 * alpha, spinnerThickness/2)
        else
             Menu.DrawRect(px - spinnerThickness/2, py - spinnerThickness/2, spinnerThickness, spinnerThickness, math.floor(r * 255), math.floor(g * 255), math.floor(b * 255), 255 * alpha)
        end
    end

    -- Calculate Key Box Position (Moved up for label logic)
    local boxSize = 34
    local boxX = math.floor(barX + barWidth - boxSize - 30)
    local boxY = math.floor(barY + (barHeight / 2) - (boxSize / 2))

    -- Label Logic
    -- Determine what text to show based on context to avoid overlap
    local itemName = Menu.BindingItem and (Menu.BindingItem.name or "Unknown") or "Toggle"
    local displayText = "Choose Menu Open Key"
    
    if itemName ~= "Toggle" and itemName ~= "Menu Toggle" and itemName ~= "Change Menu Keybind" then
        displayText = string.upper(itemName)
    end
    
    local labelSize = 16
    local textStartX = startX + (spinnerRadius * 2) + 15
    
    -- Calculate available width for text
    -- boxX is where the key box starts. We need to stop before that.
    local maxTextWidth = (boxX - 15) - textStartX
    
    -- Truncate text if too long
    local currentTextWidth = 0
    if Susano and Susano.GetTextWidth then
        currentTextWidth = Susano.GetTextWidth(displayText, labelSize)
        
        if currentTextWidth > maxTextWidth then
            -- Simple truncation approximation
            -- Remove characters until it fits
            while string.len(displayText) > 3 and currentTextWidth > maxTextWidth do
        ...
        currentTextWidth = string.len(displayText) * 8
        if currentTextWidth > maxTextWidth then
             local maxChars = math.floor(maxTextWidth / 8) - 3
             if maxChars < 1 then maxChars = 1 end
             displayText = string.sub(displayText, 1, maxChars) .. "..."
        end
    end
    
    Menu.DrawText(math.floor(textStartX), math.floor(textY - 8), displayText, labelSize, 0.9, 0.9, 0.9, 1.0 * alpha)
    
    -- Key Box (Static Black Outline)
    if Susano and Susano.DrawRectFilled then
        -- Inner Box
        Susano.DrawRectFilled(boxX, boxY, boxSize, boxSize, 0.15, 0.15, 0.15, 1.0 * alpha, 4)
    else
    ...
    local keyWidth = 0
    if Susano and Susano.GetTextWidth then
        keyWidth = Susano.GetTextWidth(keyName, keySize)
    else
        keyWidth = string.len(keyName) * 9
    end
    
    Menu.DrawText(math.floor(boxX + (boxSize/2) - (keyWidth/2)), math.floor(boxY + (boxSize/2) - (keySize/2)), keyName, keySize, 1.0, 1.0, 1.0, 1.0 * alpha)
end
...
function Menu.DrawKeybindsInterface(alpha)
            ...
            for _, tab in ipairs(cat.tabs) do
                if tab.items then
                    for _, item in ipairs(tab.items) do
                        if item.bindKey and item.bindKeyName and item.type == "toggle" then
                            table.insert(activeBinds, {
                                name = item.name,
                                keyName = item.bindKeyName,
                                isActive = item.value or false
                            })
                        end
                    end
                end
    ...
    -- New style: rounded transparent rectangle
    local padding = 15
...
function Menu.GetLayoutSegments()
    local segments = {}
    local x = Menu.Position.x
    local startY = Menu.Position.y
    local width = Menu.Position.width
    
    local headerH = Menu.Position.headerHeight
    local menuBarH = Menu.Position.mainMenuHeight
    local spacing = Menu.Position.mainMenuSpacing
    local itemH = Menu.Position.itemHeight
    local footerSpacing = Menu.Position.footerSpacing
    local footerH = Menu.Position.footerHeight
    
    -- Segment 1: Header + MenuBar (Tabs/Title)
    -- In category list view (else block of GetMenuHeight), there is a +2 offset
    local topSegmentH = headerH + menuBarH
    if not Menu.OpenedCategory then
        topSegmentH = headerH + 2 + menuBarH
    end
    
    table.insert(segments, {y = startY, h = topSegmentH})
    
    -- Segment 2: Items Body
    ...
    -- Segment 3: Footer
    -- Footer is positioned after the items (and spacing)
    local footerY = itemsY + itemsH + footerSpacing
    table.insert(segments, {y = footerY, h = footerH})
    
    -- Calculate total visual height (from top of header to bottom of footer) for gradient mapping
    local fullHeight = (footerY + footerH) - startY
...
function Menu.DrawBackground()
    local x = Menu.Position.x
    local y = Menu.Position.y
    local width = Menu.Position.width - 1 -- Match Header Width (was +1 before)
    
    local segments, fullHeight = Menu.GetLayoutSegments()

    -- Draw Gradient Background (Dark Theme Color -> Black)
    -- We draw the gradient ONLY inside the segments (skipping gaps)
    ...
    for _, seg in ipairs(segments) do
        -- Draw gradient slices within this segment
        local segSteps = math.ceil(seg.h / 2) -- 2px precision
        ...
        for i = 0, segSteps - 1 do
            ...
            -- Calculate factor based on global position
            local factor = (currentY - y) / fullHeight
            
            -- Gradient: Very Dark Theme Color -> Black
            local stepR = math.floor(r * (1 - factor) * 0.15)
            local stepG = math.floor(g * (1 - factor) * 0.15)
            local stepB = math.floor(b * (1 - factor) * 0.15)
            
            stepR = math.max(0, stepR)
            stepG = math.max(0, stepG)
            stepB = math.max(0, stepB)

            if Susano and Susano.DrawRectFilled then
                Susano.DrawRectFilled(x, currentY, width, drawH, stepR/255, stepG/255, stepB/255, 0.95, 0)
            else
                Menu.DrawRect(x, currentY, width, drawH, stepR, stepG, stepB, 240)
            end
        end
    end
    ...
    if Menu.ShowSnowflakes then
        for _, p in ipairs(Menu.Particles) do
            ...
            -- Respawn if out of bounds (bottom)
            if p.y > 1.0 then
                p.y = 0
            ...
            -- Check if particle is inside any segment
...
function Menu.Render()
    ...
    if Menu.OnRender then
        ...
        if not success then
            print("Error in Menu.OnRender: " .. tostring(err))
        end
    end
...
Menu.KeyNames = {
    ...
    [0x41] = "A", [0x42] = "B", [0x43] = "C", [0x51] = "Q", [0x45] = "E",
...
function Menu.HandleInput()
    ...
    if Menu.SelectingBind then
        ...
        local keysToCheck = {
            0x41, 0x42, 0x43, 0x51, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4A, 0x4B, 0x4C, 0x4D,
                ...
                if category and category.hasTabs and category.tabs then
                    for _, tab in ipairs(category.tabs) do
                        if tab and tab.items then
                            for _, item in ipairs(tab.items) do
                                if item and item.bindKey and item.type == "toggle" then
    ...
    if Menu.EditorMode then
        ...
        if Susano and Susano.GetCursorPos and Susano.GetAsyncKeyState then
            ...
            -- Fallback to standard GTA natives if available and Susano failed
            if not leftMouseDown and (IsControlPressed or IsDisabledControlPressed) then
                -- 24 is INPUT_ATTACK (Left Mouse Button) in GTA
    ...
    if Menu.OpenedCategory then
        ...
        if currentTab and currentTab.items then
            if Susano and Susano.GetAsyncKeyState then
                ...
                local alphaLeftWasDown = aWasDown or qWasDown
                ...
                local alphaLeftDown = (aDown == true) or (qDown == true)
                local alphaLeftState = alphaLeftDown and true or false
                if alphaLeftState then Menu.KeyStates[0x41] = true Menu.KeyStates[0x51] = true else Menu.KeyStates[0x41] = false Menu.KeyStates[0x51] = false end
                ...
                local alphaLeftPressed = (aPressed == true) or (qPressed == true)
        ...
        if Susano and Susano.GetAsyncKeyState then
            ...
            local aWasDown = Menu.KeyStates[0x41] or false
            local qWasDown = Menu.KeyStates[0x51] or false
            local leftWasDown = aWasDown or qWasDown
            local eWasDown = Menu.KeyStates[0x45] or false

            if upDown == true then Menu.KeyStates[0x26] = true else Menu.KeyStates[0x26] = false end
            if downDown == true then Menu.KeyStates[0x28] = true else Menu.KeyStates[0x28] = false end
            local leftDown = (aDown == true) or (qDown == true)
            if leftDown then Menu.KeyStates[0x41] = true Menu.KeyStates[0x51] = true else Menu.KeyStates[0x41] = false Menu.KeyStates[0x51] = false end
            if eDown == true then Menu.KeyStates[0x45] = true else Menu.KeyStates[0x45] = false end
            ...
            elseif (aPressed == true or qPressed == true) or (leftDown == true and not leftWasDown) then
                if Menu.TopLevelTabs then
                    Menu.CurrentTopTab = Menu.CurrentTopTab - 1
                    if Menu.CurrentTopTab < 1 then Menu.CurrentTopTab = #Menu.TopLevelTabs end
                    Menu.UpdateCategoriesFromTopTab()
...
function Menu.DrawInputWindow()
    ...
    -- Main Background (Dark Gray, Opaque, Rounded)
    if Susano and Susano.DrawRectFilled then
    ...
    if Susano and Susano.DrawRectFilled then
        -- White Outline (Thin) via larger rect behind
    ...
    if Susano and Susano.GetAsyncKeyState then
         ...
         -- - _
         if Menu.IsKeyJustPressed(0xBD) then -- Minus/Underscore
